...
def fetch_param(name, default):
  if rospy.has_param(name):
    ...
    rospy.logwarn(f"parameter {name} not defined. Defaulting to {default} ")
    return default
...
if __name__ == '__main__':
  ...
  g_last_twist = Twist()
  g_vel_scales[0] = fetch_param('~angular_scale', 0.3)
  g_vel_scales[1] = fetch_param('~linear_scale', 0.3)
